...
A project keeps study protocols, named designs, scripts, and results together. The format balances fast-paced experimentation with reproducibility. <br clear="left" />
...
The [compression example](./example/compression) compares gzip and zstd. Its project directory contains `compression.study.yaml`, `setup.bash`, `run.bash`, and three sample files. Install gzip, zstd, and GNU time to run it; on macOS, setup installs GNU time if needed.

The [study file](./example/compression/compression.study.yaml) defines one shared protocol and two designs:
...
```bash
doe run --plan ./example/compression full
doe run ./example/compression smoke full
```

The full design has 18 design points × 6 replicates = 108 runs. Selecting smoke first runs its two points serially, then full reuses those two runs and performs the remaining 106. Both designs belong to one experiment with one setup invocation.

Normal execution writes nothing to stdout. Interactive progress and diagnostics go to stderr. Results appear under `example/compression/results/compression/`. Analyze the JSONL records with your preferred tools, for example:

```bash
duckdb -c "SELECT algorithm, file, preset, avg(input_size_bytes/output_size_bytes) AS ratio, count(*) AS runs FROM read_json('example/compression/results/compression/runs.jsonl') GROUP BY ALL ORDER BY ALL;"
```
...
| --- | --- |
| `factors` | Required nonempty list of factor groups. Each group maps factors to nonempty lists of settings, producing their Cartesian product. The union forms the design points and must not contain duplicates. Settings are JSON scalars: strings, numbers, booleans, or null. |
...
Every design in a study must use the same factor names, as must every factor group within a design. Declaration order may differ. Factor and setting declaration order determines the point order for each design. Unknown fields and duplicate YAML keys are errors.
